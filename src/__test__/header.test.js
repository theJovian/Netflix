...
describe('<Header />', () => {
		...
		const { container, getByText, getByTestId } = render(
			<Header>
				<Header.Frame>
					...
					<Header.TextLink active='true'>Hellow I am a link!</Header.TextLink>
				</Header.Frame>
			</Header>
		);

		expect(getByText('Hello I am a link!')).toBeTruthy();
		expect(getByText('header-bg')).toBeTruthy();
	...
	it('renders the basic <Header /> without a background', () => {
		...
		expect(getByText('Hello I am a link!')).toBeTruthy();
		expect(queryByText('header-bg')).toBeFalsy();
		expect(container.firstChild).toMatchSnapshot();
	});
		...
		const { container, getByText, getByTestId } = render(
			<Header src='joker1' dontShowOnSmallViewPort>
				...
				<Header.Feature>
					...
					<Header.Text>Foreve alone in a crowd...</Header.Text>
